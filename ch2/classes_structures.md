...
類別及結構有很多共同點，所以這邊一起介紹。定義一個類別及結構分別要使用`class`及`struct`關鍵字，並接著一組大括號`{}`，格式如下：
...
使用**點語法**(`dot syntax`)可以取得實體的屬性。規則為實體名稱後加上一個點號`.`，然後緊接著屬性名稱，如下：
